<template>
  <div class="dy-slider">
    ...
    >
      ...
      <!-- <div
        class="dy-slider-step"
        v-for="item in steps"
        :key="item"
        :style="{ 'left': item + '%' }"
        v-if="showSteps"
      ></div>-->
      <div class="dy-slider-step" v-for="item in steps" :key="item" :style="{ 'left': item + '%' }"></div>
    </div>
  </div>
</template>
  ...
  computed: {
    steps() {
      ...
      return result.filter(
        step =>
          step > (100 * (this.inputValue - this.min)) / (this.max - this.min)
      );
    },
    barSize() {
